...
const monthPresencePattern =
  /\b(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\b/i

const dayPresencePattern = /\b\d{1,2}(?:st|nd|rd|th)?\b/
...
function hasDateSignal(line: string): boolean {
  return monthPresencePattern.test(line) || dayPresencePattern.test(line)
}

function shouldSkipLine(line: string): boolean {
  const lower = line.toLowerCase()
  if (!lower) return true
  const containsDateHint = hasDateSignal(line)
  if (lower.startsWith('tentative') && !containsDateHint) return true
  if (lower.startsWith('week dates') && !containsDateHint) return true
  if (lower.startsWith('week') && !/\d/.test(lower) && !containsDateHint) return true
  if (lower.startsWith('dates') && !/\d/.test(lower) && !containsDateHint) return true
  if (lower.startsWith('class activity') && !containsDateHint) return true
  if (lower.startsWith('labs') && !/\d/.test(lower) && !containsDateHint) return true
  return false
}
